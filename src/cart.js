...
let generateCartItems = () => {
  if (basket.length !== 0) {
        ...
        <div class="details">
          ...
          <div class="title-price-x">
            <h4 class="title-price">
              <p>${name}</p>
              <p class="cart-item-price">ლარი ${price}</p>
            </h4>
    ...
    <a href="Shopping.html">
      <button class="HomeBtn">პროდუქტების გვერდზე დაბრუნება</button>
    </a>
    `;
  }
};
...
let TotalAmount = () => {
  if (basket.length !== 0) {
    ...
    <h2>საერთო თანხა : ლარი ${amount}</h2>
    <button class="checkout">გადახდა</button>
    <button onclick="clearCart()" class="removeAll">კალათის გაწმენდა</button>
    `);
  } else return;
};
